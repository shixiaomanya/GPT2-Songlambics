...
class GPT2Config(PretrainedConfig):
    ...
    def __init__(
        self,
        vocab_size=32557,    # vocab_guwen.txt有32044个，这和config/config.json里的vocab_size一致
        n_positions=1024,
        n_sentences=1032,    # sentence.txt 1026个
        n_finals=36,     # finals.txt有34个
        n_pos=1032,
        n_pingze=16,    # pingze.txt有15行
        n_ctx=512,
        n_embd=768,
        n_layer=12,
        n_head=12,
        ...
        bos_token_id=32556,    # 虽然不知道这是啥还是按照原来少1吧
        eos_token_id=32556,
...
class GPT2Model(GPT2PreTrainedModel):
    def   __init__(self, config):
        ...
        self.wbe = nn.Embedding(config.n_pingze, config.n_embd)

        
        self.drop = nn.Dropout(config.embd_pdrop)    # embd_pdrop=0.1,好像是默认值，GPT2Config类的默认初始值
    ...
    def forward(
        self,
        input_ids=None,    # = batch_inputs_token
        past_key_values=None,
        attention_mask=None,
        token_type_ids=None,
        position_ids=None,
        sentence_ids=None,  # added    = batch_inputs_sentence
        final_ids=None,  # added     = batch_inputs_final
        pos_ids=None,  # added inner sentence relative position    = batch_inputs_pos
        pingze_ids=None,     # = batch_inputs_beat
        head_mask=None,
        inputs_embeds=None,
        ...
        **kwargs,
        ...
        elif input_ids is not None:           # 走这里
        ...
        if pingze_ids is not None:
            beat_ids = pingze_ids.view(-1, input_shape[-1])
        ...
        if position_ids is None:
            device = input_ids.device if input_ids is not None else inputs_embeds.device   # 走这里
        ...
        if pingze_ids is not None:
            pingze_embeds = self.wbe(pingze_ids)
            hidden_states += pingze_embeds
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        if "past" in kwargs:
            warnings.warn(      # 这里也有错误，但是并没有执行，没有多余的含past的参数输入
                "The `past` argument is deprecated and will be removed in a future version, use `past_key_values` instead.",
                FutureWarning,
            )
            past_key_values = kwargs.pop("past")
        ...
        transformer_outputs = self.transformer(
            ...
            position_ids=position_ids,
            sentence_ids=sentence_ids,          # 添加的
            final_ids=final_ids,                # 添加的
            pos_ids=pos_ids,                    # 添加的
            pingze_ids=pingze_ids,                  # 添加的
            head_mask=head_mask,
        ...
        if not return_dict:
            ...
            return ((loss,) + output) if loss is not None else output      # 看来都是走的这个return

        return CausalLMOutputWithPastAndCrossAttentions(    # 这里有问题都不报错，看来没有走这个return
            loss=loss,
            logits=lm_logits,
